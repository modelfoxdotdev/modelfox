use html::{component, html, style};
use tangram_charts::{
	bar_chart::{BarChartPoint, BarChartSeries},
	box_chart::{BoxChartPoint, BoxChartSeries, BoxChartValue},
	components::{BarChart, BoxChart},
};
use tangram_ui as ui;

#[component]
pub fn ProductionStats() {
	let bar_chart_series = vec![
		BarChartSeries {
			color: ui::colors::BLUE.to_owned(),
			data: vec![
				BarChartPoint {
					label: "asymptomatic".to_owned(),
					x: 0.0,
					y: Some(0.4752),
				},
				BarChartPoint {
					label: "atypical angina".to_owned(),
					x: 1.0,
					y: Some(0.165),
				},
				BarChartPoint {
					label: "non-angina pain".to_owned(),
					x: 2.0,
					y: Some(0.2838),
				},
				BarChartPoint {
					label: "typical angina".to_owned(),
					x: 3.0,
					y: Some(0.07591),
				},
			],
			title: Some("Training".to_owned()),
		},
		BarChartSeries {
			color: ui::colors::GREEN.to_owned(),
			data: vec![
				BarChartPoint {
					label: "asymptomatic".to_owned(),
					x: 0.0,
					y: Some(0.0),
				},
				BarChartPoint {
					label: "atypical angina".to_owned(),
					x: 1.0,
					y: Some(0.1622),
				},
				BarChartPoint {
					label: "non-angina pain".to_owned(),
					x: 2.0,
					y: Some(0.2903),
				},
				BarChartPoint {
					label: "typical angina".to_owned(),
					x: 3.0,
					y: Some(0.07508),
				},
			],
			title: Some("Production".to_owned()),
		},
	];
	let box_chart_series = vec![
		BoxChartSeries {
			color: ui::colors::BLUE.to_owned(),
			data: vec![BoxChartPoint {
				label: "exercise_max_heart_rate".to_owned(),
				x: 0.0,
				y: Some(BoxChartValue {
					max: 202.0,
					min: 71.0,
					p25: 133.5,
					p50: 153.0,
					p75: 167.5,
				}),
			}],
			title: Some("Training".to_owned()),
		},
		BoxChartSeries {
			color: ui::colors::GREEN.to_owned(),
			data: vec![BoxChartPoint {
				label: "exercise_max_heart_rate".to_owned(),
				x: 0.0,
				y: Some(BoxChartValue {
					max: 196.2,
					min: 59.0,
					p25: 120.5,
					p50: 143.1,
					p75: 177.5,
				}),
			}],
			title: Some("Production".to_owned()),
		},
	];
	html! {
		<div class="index-step">
			<div>
				<div class="index-step-title">{"Monitor data drift."}</div>
				<div class="index-step-text">
				{"After logging predictions, you can view stats comparing the production data with the training data. The app will automatically alert you if a significant difference is detected."}
				</div>
			</div>
			<ui::Window padding={Some(true)}>
				<div class="production-stats-wrapper">
					<div style={style! { "grid-area" => "number-alert" }}>
						<ui::Alert level={ui::Level::Success}>{"All Good"}</ui::Alert>
					</div>
					<div style={style! { "grid-area" => "number" }}>
						<ui::Card>
							<BoxChart
								id?="production-stats-number"
								series?={Some(box_chart_series)}
								title?="exercise_max_heart_rate"
								should_draw_x_axis_labels?={Some(false)}
							/>
						</ui::Card>
					</div>
					<div style={style! { "grid-area" => "enum-alert" }}>
						<ui::Alert level={ui::Level::Warning}>{"High Invalid Count"}</ui::Alert>
					</div>
					<div style={style! { "grid-area" => "enum" }}>
						<ui::Card>
							<BarChart
								id?="production-stats-enum"
								series?={Some(bar_chart_series)}
								title?="chest_pain"
								x_axis_title?="chest_pain"
								y_axis_title?="Percent"
								y_max?={Some(1.0)}
								y_min?={Some(0.0)}
							/>
						</ui::Card>
					</div>
				</div>
			</ui::Window>
		</div>
	}
}
