...
fn boot_tuning() {
	...
	fn update_tuning(selected_threshold_metrics: &ThresholdMetrics) {
		...
		ui::update_number("tuning-accuracy", accuracy);
		ui::update_number("tuning-precision", precision);
		ui::update_number("tuning-recall", recall);
		let code = format!("// Update your code to use the selected threshold.\nmodel.predict(input, {{ threshold: {:.2} }})", selected_threshold_metrics.threshold);
		ui::update_code("tuning-code", code);
	}
}
