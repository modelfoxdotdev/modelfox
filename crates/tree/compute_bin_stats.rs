	...
	if hessians_are_constant {
		match binned_feature_column {
			BinnedFeaturesColumnMajorColumn::U8(binned_feature_values) => unsafe {
				compute_bin_stats_column_major_inner::<u8, false, ROOT>(
					gradients,
					hessians,
			...
			BinnedFeaturesColumnMajorColumn::U16(binned_feature_values) => unsafe {
				compute_bin_stats_column_major_inner::<u16, false, ROOT>(
					gradients,
					hessians,
					binned_feature_values,
					bin_stats_for_feature,
		...
		match binned_feature_column {
			BinnedFeaturesColumnMajorColumn::U8(binned_feature_values) => unsafe {
				compute_bin_stats_column_major_inner::<u8, true, ROOT>(
					gradients,
					hessians,
					binned_feature_values,
					bin_stats_for_feature,
			...
			BinnedFeaturesColumnMajorColumn::U16(binned_feature_values) => unsafe {
				compute_bin_stats_column_major_inner::<u16, true, ROOT>(
					gradients,
					hessians,
					binned_feature_values,
					bin_stats_for_feature,
...
unsafe fn compute_bin_stats_column_major_inner<T, const HESSIANS: bool, const ROOT: bool>(
	gradients: &[f32],
	hessians: &[f32],
	...
	T: ToPrimitive,
{
	if ROOT {
		...
		for i in 0..len {
			...
			let hessian = if HESSIANS {
				*hessians.get_unchecked(i) as f64
			} else {
				1.0
			};
			bin_stats.sum_hessians += hessian;
		}
	} else {
		...
		for i in 0..prefetch_len {
			...
			let bin_stats = bin_stats_for_feature.get_unchecked_mut(bin_index);
			bin_stats.sum_gradients += ordered_gradient as f64;
			let ordered_hessian = if HESSIANS {
				*hessians.get_unchecked(i)
			} else {
				1.0
			};
	...
	if hessians_are_constant {
		match binned_features {
			BinnedFeaturesRowMajor::U16(binned_features) => unsafe {
				...
				compute_bin_stats_row_major_inner::<u16, false, ROOT>(
					bin_stats,
					examples_index,
			...
			BinnedFeaturesRowMajor::U32(binned_features) => unsafe {
				let n_features = binned_features.values_with_offsets.ncols();
				compute_bin_stats_row_major_inner::<u32, false, ROOT>(
					bin_stats,
					examples_index,
					binned_features.values_with_offsets.as_slice().unwrap(),
					gradients,
		...
		match binned_features {
			BinnedFeaturesRowMajor::U16(binned_features) => unsafe {
				let n_features = binned_features.values_with_offsets.ncols();
				compute_bin_stats_row_major_inner::<u16, true, ROOT>(
					bin_stats,
					examples_index,
					binned_features.values_with_offsets.as_slice().unwrap(),
					gradients,
			...
			BinnedFeaturesRowMajor::U32(binned_features) => unsafe {
				let n_features = binned_features.values_with_offsets.ncols();
				compute_bin_stats_row_major_inner::<u32, true, ROOT>(
					bin_stats,
					examples_index,
					binned_features.values_with_offsets.as_slice().unwrap(),
					gradients,
...
pub unsafe fn compute_bin_stats_row_major_inner<T, const HESSIANS: bool, const ROOT: bool>(
	bin_stats: &mut [BinStatsEntry],
	examples_index: &[u32],
	binned_feature_values: &[T],
	gradients: &[f32],
	...
	T: ToPrimitive,
{
	if ROOT {
		for example_index in examples_index {
			...
			let hessian = if HESSIANS {
				*hessians.get_unchecked(example_index) as f64
			} else {
				1.0
			};
