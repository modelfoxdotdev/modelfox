...
impl Component for TextColumn {
	fn into_node(self) -> Node {
		...
		let table_section = ui::S2::new().child(ui::H2::new(table_section_title)).child(
			ui::Table::new()
				.width("100%".to_owned())
				.child(
					ui::TableHeader::new()
				...
				.child(
					ui::TableBody::new().children(self.ngrams_table_rows.iter().map(
						|ngram_table_row| {
							ui::TableRow::new()
								.child(
									ui::TableCell::new().child(ngram_table_row.ngram.to_string()),
								)
								.child(
									ui::TableCell::new().child(ngram_table_row.count.to_string()),
								)
						},
					)),
				),
