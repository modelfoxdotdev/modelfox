...
pub fn main() {
	...
	if document.get_element_by_id("prediction_count").is_some() {
		hydrate::<modelfox_charts::components::BarChart>("prediction_count");
	}
	if document.get_element_by_id("quantiles_overall").is_some() {
		hydrate::<modelfox_charts::components::BoxChart>("quantiles_overall");
	}
	if document.get_element_by_id("quantiles_intervals").is_some() {
		hydrate::<modelfox_charts::components::BoxChart>("quantiles_intervals");
	}
	if document.get_element_by_id("histogram_overall").is_some() {
		hydrate::<modelfox_charts::components::BarChart>("histogram_overall");
	}
	if document.get_element_by_id("histogram_intervals").is_some() {
		hydrate::<modelfox_charts::components::BarChart>("histogram_intervals");
	}
}
